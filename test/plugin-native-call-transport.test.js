...
test('canonicalizes proven native miscalls while declared edit calls keep their identity', async (t) => {
  ...
  const persistedSession = { id: 'canonical-provenance-session', events: [] }
  ...
  assert.match(projected.latestRun.source, /Generated by PTC Plus from the model's out-of-surface "read" call/)
  assert.ok(projected.latestRun.source.includes(JSON.stringify(nativeArguments)))
