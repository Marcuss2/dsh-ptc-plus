...
test('canonicalizes proven native miscalls while declared edit calls keep their identity', async (t) => {
  ...
  assert.ok(qualifiedArguments.code.includes(JSON.stringify(nativeArguments)))
  ...
  for (const deltaChunk of parallelDeltas) {
    ...
    assert.ok(argumentsValue.code.includes(JSON.stringify(nativeArguments)))
  }
...
test('preserves both as a distinct PTC presentation across native capability views', async (t) => {
  ...
  for (const { id, nativeSchemas } of [
    ...
    const rewrite = projected.contexts.find(context => context.name === 'tools:ptc-plus-rewrite-info')
    assert.match(rewrite.text, /stripped the export modifier from a top-level declaration/)
    assert.match(rewrite.text, /reusing its ordinary top-level bindings/)
