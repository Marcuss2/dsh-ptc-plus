...
test('presents one coherent persistent REPL contract to the model', async (t) => {
  ...
  assert.match(guidance, /After a failure, start with a new short `run_code` cell/)
  assert.match(guidance, /state: partially-applied/)
  assert.match(guidance, /never edit or replay it/)
  assert.match(guidance, /safely rerun the complete cell\. It does not resume at the failing line/)
  assert.match(guidance, /explicit named checkpoints/)
