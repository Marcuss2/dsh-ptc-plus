...
```sh
dsh plugin --profile <profile> add dsh-ptc-plus@0.2.1
...
```sh
npm pack
dsh plugin --profile <profile> add /absolute/path/to/dsh-ptc-plus-0.2.1.tgz
dsh --profile <profile> --dump-config
```
...
After an npm release, the package spec may instead be `dsh-ptc-plus@0.2.1`. For a local package, use its absolute tarball path. Restart DSH Desktop after installation. Linux Desktop is not a current DSH Desktop release target; use DSH CLI/Web on Linux.
